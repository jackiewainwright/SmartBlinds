...
float AngleStateP[] = { 15 , 0 };    // 20 degree slat angle starting out  (I'd say 0 if you want it perpendicular)
float HeightStateT[] = { 0 , 0 };    // 0 means fully lowered
float AngleStateT[] = { 15 , 0 };    // 20 degree slat angle starting out
...
void setup() {
  ...
  //this should get the slats to about perpendicular. Initial movement is 60deg from switch.
  // Actually I had a lot of trouble adjusting this. I give up, initial angle is 15deg 
  for(int x = 0; x < 16*200/360*90; x++) {   
...
// now to determine the angle and blind height difference 
    if(StateSelect == 1){
      ...
      while(StateSelect == 1){
        ...
        Serial.print(Sensor_Data[4]); Serial.print(" ");
...
//          Serial.println(HeightDiff);
          ...
          if(AngleDiff < 0.0){
            digitalWrite(dirPin_3,HIGH);                  // b
            numSteps = 16*round(((0-1)*AngleDiff)*1.636/1.9); 
            ...
            digitalWrite(dirPin_3,LOW);                   // b
            numSteps = 16*round((AngleDiff)*1.636/1.9);   
      ...
      if(p >= 5){
        ...
        if(y == 0){
          ...
          if(AngleDiff < 0.0){
            digitalWrite(dirPin_3,HIGH);                // b
            numSteps = 16*round(((0-1)*AngleDiff)/1.9);
            for(int x = 0; x < numSteps; x++) {
              digitalWrite(stepPin_3,HIGH); 
              delayMicroseconds(ROTDELAY); 
              digitalWrite(stepPin_3,LOW); 
            ...
            digitalWrite(dirPin_3,LOW);                 // b
            numSteps = 16*round((AngleDiff)/1.9);
            for(int x = 0; x < numSteps; x++) {
              digitalWrite(stepPin_3,HIGH); 
              delayMicroseconds(ROTDELAY); 
              digitalWrite(stepPin_3,LOW); 
    ...
    // (Peter) This doesn't change the slat angles, which seems like a mistake
